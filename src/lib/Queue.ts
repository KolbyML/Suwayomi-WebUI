...
export class Queue {
    ...
    cancel(key: string, reason: Error = new Error('Queue::cancel: called')): boolean {
        const promise = this.pendingKeyToPromiseMap.get(key);
        if (!promise) {
            return false;
        }
    ...
    private getNextItemToProcess<T>(): { key: Key; fn: QueueItemFunction<T>; promise: ControlledPromise<T> } | null {
        const nextEntry = [...this.pendingKeyToPriorityMap.entries()].toSorted(
