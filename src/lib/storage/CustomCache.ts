...
export class CustomCache {
    ...
    public getResponseForKey<Response = any>(key: string): Response | undefined {
        return this.keyToResponseMap.get(key) as Response | undefined;
    }

    public cacheResponseForKey(key: string, response: unknown) {
        this.keyToFetchTimestampMap.set(key, Date.now());
        this.keyToResponseMap.set(key, response);
    }
