...
export const SelectSetting = <SettingValue extends string | number>({
    ...
    const fallbackDisplayInfo: SelectSettingValueDisplayInfo = useMemo(
        () => values[0]?.[1] ?? { text: '' },
        [values],
    );
    const valueDisplayText = useMemo(
        () => values.find(([key]) => key === value)?.[1]?.text ?? fallbackDisplayInfo.text,
        [value, values, fallbackDisplayInfo.text],
    );
    const dialogValueDisplayInfo = useMemo(
        () => values.find(([key]) => key === dialogValue)?.[1] ?? fallbackDisplayInfo,
        [dialogValue, values, fallbackDisplayInfo],
    );

    useEffect(() => {
        const hasValueInOptions = values.some(([optionValue]) => optionValue === value);
