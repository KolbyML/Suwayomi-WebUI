...
const READER_MANGA_ID_REGEX = /^\/manga\/([0-9]+)\/chapter\/[0-9]+(?:\/.*)?$/;
...
export const useBackButton = () => {
    ...
    return useCallback(() => {
        ...
        const getDelta = (historyToCheck: string[] = history, delta: number = 0) => {
            ...
            const ignorePreviousPage = PAGES_TO_IGNORE.some((page) => page.test(previousPage));
        ...
        if (!canNavigateBack) {
            ...
            const readerMatch = location.pathname.match(READER_MANGA_ID_REGEX);
            if (readerMatch) {
                navigate(AppRoutes.manga.path(Number(readerMatch[1])));
                return;
            }
