...
import { BackupRestoreState, ValidateBackupResult } from '@/lib/requests/types.ts';
...
export function Backup() {
    ...
    const createBackup = async () => {
        makeToast('Creating Manatan backup...', 'info');

        try {
            const backupFileResponse = await requestManager.createBackupFile({ format: 'manatanbk' }).response;

            const backupFileUrl = backupFileResponse.data?.createBackup.url;
            if (!backupFileUrl) {
                makeToast('Failed to create Manatan backup', 'error', getErrorMessage(backupFileResponse.errors));
                return;
            }
            ...
            makeToast('Failed to create Manatan backup', 'error', getErrorMessage(e));
        }
    };

    const validateBackup = async (file: File) => {
        const promptAddExtensionRepos = async () => {
            try {
                await AwaitableComponent.show(
                    ConfirmDialog,
                    {
                        title: t('extension.action.label.install'),
                        message: t('extension.label.add_repository_info'),
                        actions: {
                            extra: {
                                show: true,
                                title: t('settings.title'),
                                contain: true,
                                link: AppRoutes.settings.childRoutes.browse.path,
                            },
                            confirm: {
                                show: false,
                            },
                    ...
                    { id: 'backup-missing-extension-repos' },
                );
            } catch (_) {
                // ignore
            }
        ...
        const installMissingSources = async (validationResult: ValidateBackupResult) => {
            if (!validationResult.missingSources.length) {
                return;
            }

            const extensionRepos = settingsData?.settings?.extensionRepos ?? [];
            if (!extensionRepos.length) {
                await promptAddExtensionRepos();
                return;
            }

            try {
                const response = await requestManager
                    .installBackupMissingSourceExtensions(validationResult.missingSources)
                    .response;
                const result = response.data?.installBackupMissingSources;
                const installedCount = result?.installedPkgNames?.length ?? 0;
                const failedCount = result?.failedPkgNames?.length ?? 0;
                const unresolvedCount = result?.unresolvedSources?.length ?? 0;

                if (installedCount > 0) {
                    makeToast(t('extension.label.installed_successfully'), 'success');
                }

                if (failedCount || unresolvedCount) {
                    makeToast(
                ...
                makeToast(t('extension.action.label.install'), 'error', getErrorMessage(e));
            }
        };
                ...
                try {
                    await AwaitableComponent.show(
                        BackupValidationDialog,
                        {
                            ...
                            onInstallMissingSources: installMissingSources,
                        },
    ...
    const submitBackup = async (file: File) => {
        ...
        const isValidFilename =
            lowerName.endsWith('.manatanbk') ||
            lowerName.endsWith('.tachibk') ||
            lowerName.endsWith('.proto.gz');
        if (!isValidFilename) {
        ...
        <>
            <List sx={{ padding: 0 }}>
                <ListItemButton onClick={createBackup}>
                    <ListItemText
                        primary="Create Manatan backup"
                        secondary="Export a .manatanbk backup for manga, anime, and novel data."
                    />
                </ListItemButton>
            ...
            <input
                ref={mergedInputRef}
                type="file"
                accept=".manatanbk,.tachibk,.proto.gz"
                style={{ display: 'none' }}
            />
        </>
    );
