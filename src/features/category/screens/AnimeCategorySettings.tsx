...
import { CategorySettingFieldsFragment } from '@/lib/requests/types.ts';
...
import { AnimeCategorySettingsCard } from '@/features/category/components/AnimeCategorySettingsCard.tsx';
import { CreateOrEditAnimeCategoryDialog } from '@/features/category/components/CreateOrEditAnimeCategoryDialog.tsx';

export function AnimeCategorySettings() {
    const { t } = useTranslation();
    const dndSensors = DndKitUtil.useSensorsForDevice();

    useAppTitle('Video Categories');

    const { data, loading, error, refetch } = requestManager.useGetAnimeCategoriesSettings({
    ...
    const [categories, setCategories] = useState<CategorySettingFieldsFragment[]>([]);
    const allCategories = data?.categories.nodes ?? [];
    const categoriesFromData = useMemo(() => allCategories.filter((category) => category.id !== 0), [allCategories]);
    ...
    const handleCategoryDeleted = (id: number) => {
        ...
        refetch().catch(defaultPromiseErrorHandler('AnimeCategorySettings::refetchAfterDelete'));
    };
    ...
    return (
        <>
            ...
            {dialogOpen && (
                <CreateOrEditAnimeCategoryDialog
                    category={categories[categoryToEdit]}
                    onClose={handleDialogCancel}
                    onSaved={() => refetch().catch(defaultPromiseErrorHandler('AnimeCategorySettings::refetch'))}
                />
            )}
        </>
    );
