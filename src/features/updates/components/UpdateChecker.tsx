...
export function UpdateChecker({
    ...
    const lastRunningStateRef = useRef(false);
    ...
    const status =
        updaterSubscriptionData?.libraryUpdateStatusChanged ?? updaterData?.libraryUpdateStatus;

    const isRunning = !!status?.jobsInfo.isRunning;
    const progress =
        status && status.jobsInfo.totalJobs > 0 ? (status.jobsInfo.finishedJobs / status.jobsInfo.totalJobs) * 100 : 0;

    useEffect(() => {
        if (!lastRunningStateRef.current && isRunning) {
            lastRunningStateRef.current = true;
        }

        const isUpdateFinished = lastRunningStateRef.current && !isRunning;
        if (!isUpdateFinished) {
            return;
        }

        lastRunningStateRef.current = false;
        handleFinishedUpdate?.();
    ...
    }, [isRunning, handleFinishedUpdate, reFetchLastTimestamp]);
