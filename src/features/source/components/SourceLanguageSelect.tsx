...
export const SourceLanguageSelect = ({
    ...
    const allLanguagesSelected =
        !!languagesSortedBySelectState.length &&
        languagesSortedBySelectState.every((language) => tmpSelectedLanguages.includes(language));
    ...
    const handleOk = () => {
        ...
        Promise.all(
            Object.entries(sourceIdToEnabledState).map(([sourceId, enabled]) => {
                ...
                return updateSourceMetadata(source, 'isEnabled', enabled, sourceScope);
            }),
        )
            .then(() => onMetaUpdated?.())
    ...
    return (
        <>
            <CustomTooltip title={t('global.button.filter')}>
            ...
            <Dialog fullWidth maxWidth="xs" open={open} onClose={handleCancel}>
                ...
                <DialogActions>
                    ...
                    >
                        <Button
                            onClick={() =>
                                setTmpSelectedLanguages(allLanguagesSelected ? [] : languagesSortedBySelectState)
                            }
                        >
                            {t(allLanguagesSelected ? 'global.button.deselect' : 'global.button.select_all')}
                        </Button>
                        <Stack direction="row">
