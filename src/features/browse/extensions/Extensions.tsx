...
import { filterBrowsableMangaExtensions } from '@/features/browse/extensions/animeExtensionFilter.ts';
...
export function Extensions({ tabsMenuHeight }: { tabsMenuHeight: number }) {
    ...
    const {
        metadata,
        settings: { sourceLanguages, showNsfw },
    } = useMetadataServerSettings();
    ...
    const { shouldSuppressError } = useTransientResumeRetry({
        ...
        hasData: Boolean(data?.fetchExtensions?.extensions.length),
    ...
    const repoExtensions = useMemo(() => filterBrowsableMangaExtensions(allExtensions ?? []), [allExtensions]);
