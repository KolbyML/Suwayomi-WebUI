...
import { MANATAN_NAVBAR_COLLAPSED_META_KEY, useServerMetaState } from '@/Manatan/services/ServerMetaStorage.ts';

const NAVBAR_COLLAPSED_LOCAL_STORAGE_KEY = 'NavBar::isCollapsed';
...
export function NavBarContextProvider({ children }: IProps) {
    const [title, setTitle] = useState<string | React.ReactNode>('Manatan');
    ...
    const [isCollapsed, setIsCollapsed] = useServerMetaState(MANATAN_NAVBAR_COLLAPSED_META_KEY, false, {
        legacyLocalStorageKey: NAVBAR_COLLAPSED_LOCAL_STORAGE_KEY,
    });
    ...
    const updateTitle = useCallback(
        (newTitle: string | React.ReactNode, browserTitle: string = typeof newTitle === 'string' ? newTitle : '') => {
            const resolvedTitle = browserTitle.trim() || 'Manatan';
