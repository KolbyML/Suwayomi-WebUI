...
export const ChapterActionMenuItems = ({
    ...
    const hasOcrData = isOcrProcessed || isOcrProcessing || (ocrStatus?.status === 'idle' && ocrStatus.cached > 0);
        ...
        <>
            ...
            {!isSingleMode && selectModeOcrState.showDownloadAndOcr && (
                <MenuItem
                    ...
                    onClick={() => {
                        const chaptersToDownloadAndOcr = selectModeOcrState.chaptersNeedingOcr.filter((candidate) =>
                            downloadableChapters.some((downloadableChapter) => downloadableChapter.id === candidate.id),
                        );
                        const downloadedChaptersToOcr = selectModeOcrState.chaptersNeedingOcr.filter(
                            (candidate) =>
                                candidate.isDownloaded &&
                                !chaptersToDownloadAndOcr.some((queued) => queued.id === candidate.id),
                        );
                        ...
                        Chapters.performAction('download', Chapters.getIds(downloadableChapters), {
                            ...
                            .then(() => {
                                downloadedChaptersToOcr.forEach((c) => {
                                    const path = `/manga/${c.mangaId}/chapter/${c.sourceOrder}`;
                                    void startChapterOcr(path);
                                });
                                chaptersToDownloadAndOcr.forEach((c) => {
                                    const path = `/manga/${c.mangaId}/chapter/${c.sourceOrder}`;
                                    queueChapterOcrAfterDownload(path, c.id);
                                });
                            })
                ...
                <MenuItem
                    ...
                    onClick={() => {
                        Chapters.performAction('download', [chapter!.id], {
                            ...
                            .then(() => {
                                queueChapterOcrAfterDownload(chapterPath, chapter!.id);
                            })
                            .catch(defaultPromiseErrorHandler('ChapterActionMenuItems::downloadAndOcr'));
                        onClose();
                    }}
