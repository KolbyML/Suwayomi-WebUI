...
import { detectTextBoxMobilePlatform } from '@/Manatan/utils/textBoxMobilePlatform.ts';
...
export const ReaderBehaviourSettings = ({
    ...
    const canUseAndroidVolumeKeys =
        typeof window !== 'undefined' && detectTextBoxMobilePlatform() === 'android-native-webview';
    ...
    return (
        <Stack sx={{ gap: 2 }}>
            ...
            />
            {canUseAndroidVolumeKeys && (
                <>
                    <CheckboxInput
                        label={t('reader.settings.volume_keys.title')}
                        checked={settings.readWithVolumeKeys}
                        onChange={(_, checked) => updateSetting('readWithVolumeKeys', checked)}
                    />
                    <CheckboxInput
                        label={t('reader.settings.volume_keys.inverted')}
                        checked={settings.readWithVolumeKeysInverted}
                        onChange={(_, checked) => updateSetting('readWithVolumeKeysInverted', checked)}
                        disabled={!settings.readWithVolumeKeys}
                    />
                </>
            )}
            <CheckboxInput
