...
import { useSyncOnChapterRead } from '@/features/sync/services/useSyncTriggers.ts';
...
export class ReaderService {
    ...
    static useUpdateChapter(): (patch: UpdateChapterPatchInput) => void {
        const {
            settings: { deleteChaptersWhileReading, deleteChaptersWithBookmark },
        ...
        return useCallback(
            (patch) => {
                ...
                const update = async () => {
                    ...
                    try {
                        await requestManager.updateChapters(
                            chapterIdsToUpdate,
                            {
                                ...patch,
                                chapterIdsToDelete,
                                trackProgressMangaId:
                                    (patch.isRead !== undefined || patch.lastPageRead !== undefined) &&
                                    manga.trackRecords.totalCount
                                        ? manga.id
                                        : undefined,
                            },
                            { errorPolicy: 'all' },
                        ...
                        if (patch.isRead !== undefined || patch.lastPageRead !== undefined) {
                            triggerSync();
                        }
                    } catch (error) {
                        defaultPromiseErrorHandler('ReaderService::useUpdateChapter')(error);
                    }
                };

                ReaderService.getOrCreateChapterUpdateQueue(currentChapter.id).enqueue(`${currentChapter.id}`, update);
            },
            [deleteChaptersWhileReading, deleteChaptersWithBookmark, triggerSync],
        );
    }
