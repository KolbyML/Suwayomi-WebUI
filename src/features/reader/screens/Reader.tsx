...
import { setLastMangaResumeTarget } from '@/features/navigation-bar/NavigationBarResume.util.ts';
import { useSyncOnChapterOpen } from '@/features/sync/services/useSyncTriggers.ts';
...
import { ReaderZoomProvider } from '@/features/reader/zoom/ReaderZoomContext.tsx';
...
const BaseReader = ({
    ...
    useAppTitle(
        !manga || !currentChapter ? t('reader.title', { mangaId, chapterIndex: chapterSourceOrder }) : manga.title,
    );
    ...
    useEffect(() => {
        if (!Number.isFinite(mangaId) || !Number.isFinite(chapterSourceOrder)) {
            return;
        }

        setLastMangaResumeTarget(mangaId, chapterSourceOrder);
    }, [mangaId, chapterSourceOrder]);
    useSyncOnChapterOpen(Number.isFinite(mangaId) && currentChapter ? `manga:${mangaId}:${currentChapter.id}` : null);
    ...
    if (isLoading) {
        return (
            <Box
                sx={{
                    height: READER_VIEWPORT_HEIGHT,
                    width: READER_VIEWPORT_WIDTH,
                    display: 'grid',
                    placeItems: 'center',
                }}
            >
        ...
        <ReaderZoomProvider>
            <Box
                sx={{
                    ...
                    minWidth: `calc(${READER_VIEWPORT_WIDTH} - ${readerNavBarWidth}px)`,
                    maxWidth: `calc(${READER_VIEWPORT_WIDTH} - ${readerNavBarWidth}px)`,
                    width: `calc(${READER_VIEWPORT_WIDTH} - ${readerNavBarWidth}px)`,
                    height: READER_VIEWPORT_HEIGHT,
