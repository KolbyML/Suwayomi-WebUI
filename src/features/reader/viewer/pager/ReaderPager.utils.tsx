...
export const getImagePlaceholderStyling = (
    ...
    const getViewportWidthPercentage = (width: number) => `calc(${READER_VIEWPORT_WIDTH} * ${width / 100})`;
    const getDesktopWidth = (width: number, readerWidthValue?: number) =>
        getMaxWidth(
            getViewportWidthPercentage(
                coerceIn(readerWidthValue ?? width, Math.min(width, readerWidthValue ?? OVER_9000), width),
            ),
        );

    const setReaderWidth = getSetReaderWidth(readerWidth, pageScaleMode);
    const fullWidth = getMaxWidth(getViewportWidthPercentage(Math.min(100, setReaderWidth ?? OVER_9000)));
    const fullHeight = `calc(${READER_VIEWPORT_HEIGHT} - ${heightOffset}px)`;

    const DEFAULT_SINGLE_PAGE_WIDTH = isTabletWidth ? fullWidth : getDesktopWidth(40, setReaderWidth);
    const DEFAULT_SINGLE_PAGE_HEIGHT = isTabletWidth ? fullHeight : `calc(${READER_VIEWPORT_HEIGHT} * 0.85)`;
    ...
    const READING_MODE_TO_PLACEHOLDER_SIZE: Record<ReadingMode, { minWidth: string; minHeight: string }> = {
        ...
        [ReadingMode.CONTINUOUS_VERTICAL]: {
            ...
            minHeight: READER_VIEWPORT_HEIGHT,
        },
...
const getReaderDimensionStyling = (
    ...
    const fullWidth = `calc((${READER_VIEWPORT_WIDTH} - ${widthOffset}px) * ${widthPercentage})`;
    const fullHeight = `calc(${READER_VIEWPORT_HEIGHT} - ${heightOffset}px)`;
...
export const createSinglePageData = (url: string, index: number): ReaderStatePages['pages'][number]['primary'] => ({
    index,
    alt: `Page #${index + 1}`,
    url: requestManager.getValidImgUrlFor(url, ''),
});
...
export const createReaderPage = (
    ...
    <ReaderPage
        ...
        alt={alt}
        display={display}
        backendPriority={index === currentPageIndex ? 'high' : 'low'}
