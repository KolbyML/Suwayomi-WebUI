...
import { shouldPreserveContinuousReaderPageOnResize } from '@/features/reader/viewer/hooks/readerResizePreservation.ts';
...
const usePreserveOnWindowResize = (
    ...
    const handleResize = useCallback(() => {
        ...
        if (
            shouldPreserveContinuousReaderPageOnResize(
...
const isReaderZoomActive = () =>
    document.body.classList.contains('reader-zooming') || document.body.classList.contains('reader-zoomed');
...
const usePreserveOnInfiniteScrollPreviousChapterInitialRender = (
    ...
    const preserveScrollPosition = (): boolean => {
        ...
        if (!scrollElement || !isContinuousReadingModeActive || !visibleElement || isReaderZoomActive()) {
            return false;
        }
...
export const useReaderPreserveScrollPosition = (
    ...
    usePreserveOnWindowResize(readingMode, setPageToScrollToIndex, currentPageIndex);
