...
import { MihonPagedReader } from '@/features/reader/viewer/mihon-pager/MihonPagedReader.tsx';
...
> &
            ...
            >
                ...
                >
                    <Button
                        variant="outlined"
                        onClick={exitReader}
                        startIcon={getOptionForDirection(<ArrowBack />, <ArrowForwardIcon />)}
                    >
                        {t('reader.button.exit')}
                    </Button>
                </Box>
            </Box>
        );
    ...
    // --- RENDER CONTENT ---
    const shouldUseMihonPager = !isContinuousReadingModeActive && isCurrentChapter && !isPreloadMode;

    if (shouldUseMihonPager) {
        return (
            <ReaderProfiler id="ReaderChapterViewer">
                <Box
                    ref={ref}
                    sx={{
                        width: '100%',
    ...
    return <ReaderProfiler id="ReaderChapterViewer">{PagerContent}</ReaderProfiler>;
};
