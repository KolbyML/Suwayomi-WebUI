...
export const ImageProcessingSetting = ({ type }: { type: ImageProcessingType }) => {
    ...
    const normalizedServerConversions = useMemo(
        () => (conversions ? normalizeConversions(maybeAddDefault(addStableIdToConversions(conversions))) : null),
        [conversions],
    );
    const [tmpConversions, setTmpConversions] = useState(() => normalizedServerConversions ?? []);
    const previousServerConversionsRef = useRef<typeof normalizedServerConversions>(null);

    useEffect(() => {
        if (!normalizedServerConversions) {
            return;
        }

        setTmpConversions((prev) => {
            const previousServerConversions = previousServerConversionsRef.current;
            previousServerConversionsRef.current = normalizedServerConversions;

            if (!prev.length || !previousServerConversions) {
                return normalizedServerConversions;
            }

            const serverChanged = didUpdateConversions(previousServerConversions, normalizedServerConversions);
            const hasUnsavedChanges = didUpdateConversions(previousServerConversions, prev);

            if (!serverChanged || hasUnsavedChanges) {
                return prev;
            }

            return normalizedServerConversions;
        });
    }, [normalizedServerConversions]);
    ...
    const hasChanged = didUpdateConversions(normalizedServerConversions, tmpConversions);
