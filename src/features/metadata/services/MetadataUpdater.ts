...
import { SourceIdInfo, SourceMetadataScope } from '@/features/source/Source.types.ts';
...
const requestUpdateMetadataValue = async (
    ...
    sourceScope?: SourceMetadataScope,
): Promise<void> => {
    ...
    switch (holderType) {
        ...
        case 'manga':
            ...
            break;
        case 'source':
            await requestManager.setSourceMeta((metadataHolder as SourceIdInfo).id, metadataKey, value, sourceScope)
                .response;
            break;
        default:
...
const requestUpdateMetadata = async (
    ...
    Promise.all(
        keysToValues.map(([key, value]) =>
            requestUpdateMetadataValue(metadataHolder, holderType, key, value, keyPrefixes, isMetadataKey, sourceScope),
        ),
    );
...
export const requestUpdateSourceMetadata = async (
    ...
    isMetadataKey?: boolean,
    sourceScope?: SourceMetadataScope,
): Promise<void[]> => requestUpdateMetadata(source, 'source', keysToValue, keyPrefixes, isMetadataKey, sourceScope);
...
export const requestDeleteMetadataValue = async (
    ...
    switch (holderType) {
        ...
        case 'manga':
            ...
            break;
        case 'source':
            await requestManager.deleteSourceMeta((metadataHolder as SourceIdInfo).id, metadataKey, sourceScope)
                .response;
            break;
        default:
...
async function requestDeleteMetadata(
    ...
    return Promise.all(
        keys.map((key) =>
            requestDeleteMetadataValue(metadataHolder, holderType, key, keyPrefixes, isMetadataKey, sourceScope),
        ),
    );
}
...
export const requestDeleteSourceMetadata = async (
    ...
    isMetadataKey?: boolean,
    sourceScope?: SourceMetadataScope,
): Promise<void[]> => requestDeleteMetadata(source, 'source', keys, keyPrefixes, isMetadataKey, sourceScope);
