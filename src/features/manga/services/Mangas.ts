...
export class Mangas {
    ...
    static getThumbnailUrl(manga: Partial<MangaThumbnailInfo>): string {
        const mangaId =
            (manga as { id?: number | string; mangaId?: number | string; manga_id?: number | string }).id ??
            (manga as { mangaId?: number | string }).mangaId ??
            (manga as { manga_id?: number | string }).manga_id;
        const isInLibrary =
            (manga as { inLibrary?: boolean; in_library?: boolean }).inLibrary ??
            (manga as { in_library?: boolean }).in_library ??
            false;
        const thumbnailUrl = manga.thumbnailUrl ?? '';
        const fallbackPath = mangaId != null ? `/api/v1/manga/${mangaId}/thumbnail` : '';
        const looksRuntimeThumbnail = thumbnailUrl.startsWith('/api/') || thumbnailUrl.includes('/api/v1/manga/');
        const shouldUseMangaThumbnailEndpoint =
            !!fallbackPath &&
            !thumbnailUrl.startsWith('data:') &&
            (isInLibrary || !thumbnailUrl || looksRuntimeThumbnail || manga.sourceId != null);
        const basePath = shouldUseMangaThumbnailEndpoint ? fallbackPath : thumbnailUrl;
        if (!basePath) {
            return '';
        }
    ...
    static async markAsUnread(mangaIds: number[], disableConfirmation?: boolean): Promise<void> {
        ...
        return Chapters.markAsUnread(
            Chapters.getIds(chapters),
            mangaIds.length === 1 ? mangaIds[0] : undefined,
            disableConfirmation,
        );
    }
    ...
    static isType(manga: MangaGenreInfo & MangaSourceNameInfo & MangaSourceLngInfo, type: MangaType): boolean {
        const sourceLanguage = typeof manga.source?.lang === 'string' ? manga.source.lang : undefined;
        const translateMangaTagsByMangaTypeEntries = Object.entries(MANGA_TAGS_BY_MANGA_TYPE).map(
            ([mangaType, tags]) => [
                mangaType,
                ['ja', i18next.language, sourceLanguage]
                    .filter((lng) => !!lng)
        ...
        const genres = Array.isArray(manga.genre) ? manga.genre : [];
        const isMatchByGenre = genres.some((genre) => {
            const normalizedGenre = typeof genre === 'string' ? genre.toLowerCase() : '';

            return (
                normalizedGenre.length > 0 &&
                translatedMangaTagsByMangaType[type].some((tag) => normalizedGenre.includes(tag.toLowerCase()))
            );
        });
        const normalizedSourceName = typeof manga.source?.name === 'string' ? manga.source.name.toLowerCase() : '';
        const isMatchBySource = SOURCES_BY_MANGA_TYPE[type].includes(normalizedSourceName);
