...
import { App } from '@/App';
import { setupBrowserConsoleForwarding } from '@/lib/debug/setupBrowserConsoleForwarding.ts';
import { installStartupPerformanceObservers, logStartupMetric } from '@/Manatan/utils/startupMetrics.ts';
...
enforceViewport();
setupBrowserConsoleForwarding();
installStartupPerformanceObservers();

logStartupMetric('browser_bundle_entry', {
...
logStartupMetric('browser_root_container_ready', {
    browserMs: Math.round(performance.now()),
    hasContainer: !!container,
});

const root = createRoot(container!);
logStartupMetric('browser_root_render_requested', {
    browserMs: Math.round(performance.now()),
});
root.render(
    <StrictMode>
